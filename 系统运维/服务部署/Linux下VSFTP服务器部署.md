...
* 客户端使用一个随机端口（Port AA）连接到FTP服务器的21号端口，完成TCP的三次握手，建立控制连接。
* 当两端需要数据发送时，客户端通过控制连接发送一个PASV command命令给服务器，要求采用被动模式进行数据传输。
* 服务器端随机选择一个端口告诉客户端，自己进行数据连接时所使用的端口号，等待客户端的连接。
* 客户端随机选择一个大于1024的端口（Port BB）连接到FTP服务器的随机端口，用于数据传输。
