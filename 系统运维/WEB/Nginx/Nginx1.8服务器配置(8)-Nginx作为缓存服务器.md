...
2. private。响应只能够作为私有的缓存(e.g., 在一个浏览器中)，不能在用户间共享。
3. no-cache。响应依然会被缓存,但是会向服务器端进行验证，no-cache等同于max-age=0.
4. no-store。在任何条件下，响应都不会被缓存(不会被缓存到本地)，并且不会被写入到客户端的磁盘里，这也是基于安全考虑的某些敏感的响应才会使用这个。
